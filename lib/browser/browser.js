...
class Browser {
  ...
  async start() {
    ...
    this.loader.start();
    this.events.miningpool = this.node.plugins.miningpool;
    this._addNode()
  }

  _addNode() {
    const url = 'https://raw.githubusercontent.com/WorldMobileCoin/wmcc-nodes/master/list/nodes.json';
      ...
      res.on("end", () => {
        try {
          ...
          for (let node of nodes)
            this.node.pool.hosts.addNode(node)
        } catch(err) {
