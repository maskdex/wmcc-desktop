...
class Render {
  ...
  removeModalbox(modalbox, fn, ...args) {
    JQuery('modalbox a[cancel], modalbox i').unbind('click').on('click', (evt) => {
      JQuery(modalbox).css('-webkit-transform', '');
      JQuery('modalbox').remove();
  ...
  latestTxs(txs) {
    ...
    for (let tx of txs) {
      html.splice(1, 0, `<tr><td><a title='${tx[0]}' class='r_goto_explorer'>${tx[0]}<a></td>`);
  ...
  appendServerList(el, list) {
    const ul = JQuery(`<ul></ul>`);
    const input = JQuery(`<input placeholder="${this.i18n._.general.custom_server}"/>`);
